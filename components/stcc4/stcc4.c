...
esp_err_t stcc4_i2c_init(i2c_port_num_t i2c_port_num)
{
...
static void stcc4_measurement_task(void *args)
{
    STCC4_value_t value = {0};
    while (1)
    {
...
void stcc4_start_measurement_task()
{
    if (stcc4_value_queue == NULL)
    {
        stcc4_value_queue = xQueueCreate(1, sizeof(STCC4_value_t));
        ESP_LOGI(LOG_TAG, "STCC4 value queue created");
    }
...
void stcc4_stop_measurement_task()
{
    if (stcc4_task_handle != NULL)
    {
        vTaskDelete(stcc4_task_handle);
        stcc4_task_handle = NULL;
        ESP_LOGI(LOG_TAG, "STCC4 measurement task stopped");
    }
    if (stcc4_value_queue != NULL)
    {
        vQueueDelete(stcc4_value_queue);
        stcc4_value_queue = NULL;
        ESP_LOGI(LOG_TAG, "STCC4 value queue deleted");
    }
}